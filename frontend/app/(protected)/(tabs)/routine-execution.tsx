...
export default function RoutineExecution() {
	...
	// Debug logging for current exercise selection
	// useEffect(() => {
	// 	if (routine && routineExecution) {
	// 		console.log("Current exercise selection:", {
	// 			currentIndex: routineExecution.currentExerciseIndex,
	// 			totalExercises: routine.exercises.length,
	// 			currentRoutineExercise: currentRoutineExercise
	// 				? {
	// 						exercise_id: currentRoutineExercise.exercise_id,
	// 						exercise_type: currentRoutineExercise.exercise_type,
	// 					}
	// 				: null,
	...
	// 			customExercisesLoaded: customExercises.length,
	// 		});
	// 	}
	// }, [routine, routineExecution, currentRoutineExercise, currentExercise, exercises.length, customExercises.length]);
	...
	useEffect(() => {
		...
		if (
			...
			console.log("Processing exercise completion:", {
				...
				currentIndex: routineExecution.currentExerciseIndex,
				totalExercises: routine.exercises.length,
				completedExercises: routineExecution.completedExercises,
			});
			...
			// Mark that we're processing a completion
			isProcessingCompletion.current = true;

			// Mark this completion as processed immediately
			processedCompletions.current.add(completionKey);

			// Clear URL parameters first to prevent re-processing
			...
			console.log("Moving to next exercise:", { nextIndex, totalExercises });
	...
	useEffect(() => {
		if (routineExecution?.showCountdown && currentExercise && currentRoutineExercise) {
			...
			if (currentExercise.isPremium && !isSubscribed && currentRoutineExercise.exercise_type === "standard") {
				// Show paywall instead of navigating to exercise
			...
			try {
				...
				console.log(
					`/(${exerciseType})/exercise&routineMode=true&routineId=${routineId}&exerciseIndex=${routineExecution?.currentExerciseIndex}`
				);
				router.push(
			...
			console.error("Current exercise or routine exercise is undefined", {
				...
				hasCurrentRoutineExercise: !!currentRoutineExercise,
				showCountdown: routineExecution?.showCountdown,
			});
			dispatch(setShowCountdown(false));
		}
	}, [
	...
	const handleStartExercise = () => {
		...
		// Custom exercises are always accessible regardless of premium status
		if (currentExercise.isPremium && !isSubscribed && currentRoutineExercise.exercise_type === "standard") {
			// Show paywall instead of starting the exercise
			dispatch(setPaywallModalPopup(true));
			return;
		}
